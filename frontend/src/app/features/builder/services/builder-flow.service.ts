...
export class BuilderFlowService {
  ...
  obtenerRecomendaciones(
    ...
    const proveedores = this.regaliaService.getProviders();
    const recomendaciones = this.regaliaService
      .getFixedPriceProducts()
      .map((producto) =>
        this.crearRecomendacion(producto, proveedores, solicitud, interpretacion),
      )
  ...
  private crearRecomendacion(producto: FixedPriceProduct,proveedores: RegaliaProvider[],solicitud: RegaliaRequest,interpretacion: InterpretacionBuilder,): RecomendacionProductoBuilder {
    const proveedor = proveedores.find((item) => item.id === producto.providerId) ?? null;
    const puntaje = this.calcularPuntaje(producto, proveedor, solicitud, interpretacion.category);
  ...
  private calcularPuntaje(
    ...
    const categoriaScore = producto.providerCategory === categoria ? 30 : 8;
    ...
    const reputacionScore = proveedor ? Math.round(proveedor.reputation / 10) : 6;
  ...
  private crearMotivo(
    ...
    if (producto.providerCategory === categoria && producto.occasion === solicitud.occasion) {
